"""Tests for data_service.oi_flush_detector — OI-based liquidation detection."""
...
class TestDetection:
    ...
    def test_no_event_on_first_snapshot(self):
        ...
        detector.update(_make_oi("BTC/USDT", 1_000_000, 1000))
        assert detector.get_recent_oi_flushes("BTC/USDT") == []
    ...
    def test_no_event_on_small_drop(self):
        ...
        detector.update(_make_oi("BTC/USDT", 990_000, ts + 300_000))
        assert detector.get_recent_oi_flushes("BTC/USDT") == []
    ...
    def test_event_on_threshold_drop(self):
        ...
        ts = int(time.time() * 1000)
        detector.update(_make_oi("BTC/USDT", 1_000_000, ts))
        detector.update(_make_oi("BTC/USDT", 980_000, ts + 300_000))
    ...
    def test_no_event_on_oi_increase(self):
        ...
        ts = int(time.time() * 1000)
        detector.update(_make_oi("BTC/USDT", 1_000_000, ts))
        detector.update(_make_oi("BTC/USDT", 1_100_000, ts + 300_000))
        assert detector.get_recent_oi_flushes("BTC/USDT") == []
...
class TestPairIsolation:
    ...
    def test_pairs_tracked_independently(self):
        ...
        detector.update(_make_oi("BTC/USDT", 1_000_000, ts))
        detector.update(_make_oi("ETH/USDT", 500_000, ts))

        # Drop BTC OI by 3%, ETH unchanged
        detector.update(_make_oi("BTC/USDT", 970_000, ts + 300_000))
        detector.update(_make_oi("ETH/USDT", 500_000, ts + 300_000))

        assert len(detector.get_recent_oi_flushes("BTC/USDT")) == 1
        assert len(detector.get_recent_oi_flushes("ETH/USDT")) == 0
    ...
    def test_filter_by_pair(self):
        ...
        detector.update(_make_oi("ETH/USDT", 500_000, ts))
        detector.update(_make_oi("BTC/USDT", 970_000, ts + 300_000))
        detector.update(_make_oi("ETH/USDT", 480_000, ts + 300_000))

        # Both pairs dropped >2%
        all_events = detector.get_recent_oi_flushes(None)
        assert len(all_events) == 2

        btc_events = detector.get_recent_oi_flushes("BTC/USDT")
...
class TestAggregatedStats:
    ...
    def test_stats_empty(self):
        detector = OIFlushDetector()
        stats = detector.get_aggregated_stats("BTC/USDT", minutes=5)
...
class TestEdgeCases:
    ...
    def test_zero_oi_ignored(self):
        detector = OIFlushDetector()
        ts = int(time.time() * 1000)
        detector.update(_make_oi("BTC/USDT", 0, ts))
        detector.update(_make_oi("BTC/USDT", 0, ts + 300_000))
        assert detector.get_recent_oi_flushes("BTC/USDT") == []
    ...
    def test_minutes_filter(self):
        ...
        detector.update(_make_oi("BTC/USDT", 1_000_000, old_ts))
        detector.update(_make_oi("BTC/USDT", 950_000, old_ts + 300_000))

        # Should not appear in last 60 minutes
        assert len(detector.get_recent_oi_flushes("BTC/USDT", minutes=60)) == 0
