...
# ============================================================
# Binance Liquidation Parsing
# ============================================================

class TestBinanceLiquidationParsing:
    """Test message parsing even though the WS is geo-blocked.
    The parser logic is still useful if we ever route via a proxy.
    """

    def _make_feed(self):
        from data_service.binance_liq import BinanceLiquidationFeed
        return BinanceLiquidationFeed()

    def test_valid_long_liquidation(self):
        feed = self._make_feed()
        msg = {
            "e": "forceOrder",
        ...
        feed._handle_force_order(msg)
        events = feed.get_recent_liquidations("BTC/USDT")
        assert len(events) == 1
        assert events[0].side == "long"
        assert events[0].size_usd == 5000.0  # 0.1 * 50000
        assert events[0].source == "binance_forceOrder"

    def test_valid_short_liquidation(self):
        feed = self._make_feed()
        msg = {
            "e": "forceOrder",
            "E": int(time.time() * 1000),
        ...
        feed._handle_force_order(msg)
        events = feed.get_recent_liquidations("ETH/USDT")
        assert len(events) == 1
        assert events[0].side == "short"
        assert events[0].pair == "ETH/USDT"

    def test_non_our_pair_filtered(self):
        feed = self._make_feed()
        msg = {
            "e": "forceOrder",
            "E": 1000,
            "o": {"s": "SOLUSDT", "S": "SELL", "q": "100", "p": "100", "T": 1000},
        }
        feed._handle_force_order(msg)
        assert len(feed._events) == 0

    def test_non_forceorder_ignored(self):
        feed = self._make_feed()
        feed._handle_force_order({"e": "trade", "data": {}})
        assert len(feed._events) == 0
    ...
    def test_aggregated_stats(self):
        ...
        # Add a long and short liquidation
        feed._events = [
            LiquidationEvent(ts, "BTC/USDT", "long", 10000.0, 50000, "binance_forceOrder"),
            LiquidationEvent(ts, "BTC/USDT", "short", 5000.0, 50000, "binance_forceOrder"),
        ]
        stats = feed.get_aggregated_stats("BTC/USDT", minutes=5)
        assert stats["total_usd"] == 15000.0
        assert stats["long_usd"] == 10000.0
        assert stats["short_usd"] == 5000.0
    ...
    def test_prune_old_events(self):
        ...
        feed._events = [
            LiquidationEvent(old_ts, "BTC/USDT", "long", 10000, 50000, "binance_forceOrder"),
            LiquidationEvent(recent_ts, "BTC/USDT", "long", 5000, 50000, "binance_forceOrder"),
        ]
        feed._prune_old_events()
        assert len(feed._events) == 1
        assert feed._events[0].timestamp == recent_ts
