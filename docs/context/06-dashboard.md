...
Página dedicada (`src/app/chart/page.tsx`, libs `src/lib/chartDatafeed.ts` + `src/lib/detectionOverlay.ts` + `src/lib/positionTool.ts`). Usa **klinecharts 9.8.12** (lazy en esta ruta; bundle ~56 kB; sparklines siguen SVG). Switchers BTC/ETH + 5m/15m/1h/4h/1D (1D usa candles `1d` del DB; 1W no almacenado), panel VOL aparte, tema Apple-dark. `chartDatafeed.ts` mapea las respuestas UDF de `/api/chart/*` (segundos) a klines (ms). Datos via `/api/chart/history`.
...
- **Position tool (A6) — `positionTool.ts`:** botones `+ Long` / `+ Short` crean un overlay custom (`positionTool`) con 3 handles arrastrables (entry/SL/TP). Pinta caja de reward verde (entry→TP) + caja de risk roja (entry→SL) hasta el borde derecho, línea de entry punteada, y labels ancladas a la derecha: `TP <px> (+x%)`, `SL <px> (-x%)`, `Entry <px> · R:R <n>`. La dirección la define la geometría (arrastrar un handle a través de entry cambia long↔short solo). Defaults: 1% riesgo / 2% reward (2R). `createPointFigures` lee los valores live de los puntos en cada repaint → R:R + caja + labels recalculan al arrastrar sin round-trip a React; `onPressedMoveEnd` refleja el R:R al chip del toolbar (el chip lo limpia). Práctica pura — sin persistencia, sin órdenes; read-only. Verificado en browser + 375px sin overflow.
- **Pendiente:** pase mobile A7 completo; tuning de knobs (Focus/significancia OB), decisión de portar significancia FVG al detector `fvg.py`, timeframe 1W. (C3 gate de fidelidad: ✅ hecho, `scripts/chart_c3_fidelity.py`.)
