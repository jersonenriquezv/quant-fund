...
- **Filtro "Significant" (de-ruido FVG, solo chart):** toggle (ON por defecto) que aplica el umbral adaptativo de LuxAlgo POR TF — un FVG es significativo si su barra de desplazamiento (`FVG.timestamp`==c2) se movió más que `2× la media corrida de |body %|` de ESE TF. El backend lo precomputa por zona (`significant`) en `detection_timeline`; el front filtra con `zonesAsOf(..., significantOnly)`. NO toca el detector del bot (`fvg.py` sigue con `FVG_MIN_SIZE_PCT` fijo) — el chart sigue siendo herramienta de validación; el filtro es solo visual. OBs siempre `significant=true`.
