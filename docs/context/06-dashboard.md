...
| `GET /api/shadow/dt?limit=100` | PG `dt_shadow_trades` (read-only) | Dual Thrust shadow (ETH 4h) — paper book propio de $10k, separado de los shadows de `ml_setups`. `DT_START_BALANCE + cumsum(pnl_net)` por `exit_ts`; max DD peak-to-trough; WR/PF. Devuelve summary + `points[]` (curva) + `trades[]` (recientes primero). Lo escribe el tracker order-free (`execution_service/dual_thrust_shadow.py`) vía `data_store.store_dt_shadow_trades` desde el hook de `main.py` (idempotente `ON CONFLICT DO NOTHING`). `available=false` si la tabla aún no existe |
| `GET /api/shadow/ml-status` | PG `ml_forward_status` (read-only) | Estado del forward-gate meta-label de engine1. Tabla single-row (`id=1, payload jsonb, updated_at`) que escribe `scripts/ml_v1_forward_check.py` (timer diario, create idempotente + upsert). Devuelve `available, n_forward/n_gate, gate_reached, verdict_state (accumulating/pass/fail), verdict, train_n, cutoff_created_at, arms {take_all, top_half, bottom_half} con {wr,pf,pnl,n}` (pf null = ∞). `available=false` si el checker nunca corrió. AUC NO se expone aquí (vive en runs ml_v0, no en este pipeline). **`milestone_n`** (+ `milestone_threshold=500`): count de outcomes binarios engine1 (tp+sl, fv≥4, TODOS los experiments) vía `queries.get_ml_training_milestone()` — espeja `scripts/alert_ml_milestone.sh`. Es el tamaño de dataset para RE-ENTRENAR, eje distinto al forward-gate (validación). Se devuelve aun si `available=false`. N_GATE subió 30→100 el 2026-06-28 (ver SB §8) |
