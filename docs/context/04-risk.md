...
- **Método principal:** `check(setup: TradeSetup, ai_confidence: float = 1.0, dry_run: bool = False) -> RiskApproval`
  ...
  3. **Dynamic position sizing via PositionSizer:** `position_size = (capital × RISK_PER_TRADE) / SL_distance`. Leverage derived from notional, capped at MAX_LEVERAGE. Falls back to FIXED_TRADE_MARGIN flat sizing if PositionSizer raises.
     - **Shadow path (dry_run + capital_override=SHADOW_CAPITAL):** if a guardrail rejects (typically `MIN_RISK_DISTANCE_PCT` for tight-SL setups), `RiskApproval.position_size=0` is returned. Live ignores this (no `execute()`). `ShadowMonitor.add_shadow()` re-runs the same risk-based formula directly so shadow tracking still gets correct sizing for ML data quality. See `docs/context/05-execution.md` § Shadow Monitor.
