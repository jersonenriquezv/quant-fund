...
## Phase 3 — ML training status panel — ✅ DONE 2026-06-23
**Decision:** option (a) via **PG row**, not a file. api container only mounts `config/.env` (no host-file/models/ volume), so a status file the checker writes on the host is unreachable. Both processes already talk to PG → single-row table `ml_forward_status (id=1, payload jsonb, updated_at)`, created idempotently + upserted by `ml_v1_forward_check.py` on every run (0-forward / accumulating / verdict paths). Best-effort write — never breaks the check. AUC NOT surfaced: it lives in the ml_v0 runs, not this frozen-model forward pipeline; panel focuses on the real-money gate (top-half PF vs take-all).
**Shipped:** `queries.get_ml_forward_status` (guards `UndefinedTableError` → null; json.loads since asyncpg returns jsonb as str), `routes/shadow.py` `GET /api/shadow/ml-status`, models `ShadowMLStatus`/`ShadowMLArm`. Frontend "ML training — engine1 forward gate" card: progress bar (N/gate, colored by state), verdict box, 3-arm table (take-all / model top-half / model bottom-half: N/WR/PF/PnL; PF null=∞), last-check ts. Empty state when checker never ran. Tests +4 (missing-table, missing-row, jsonb-string decode, decoded-dict) → 13 total pass. Mobile 375px verified. Live: cutoff 2026-06-23, train N=322, forward 6/30 accumulating.

## Phase 4 — DT shadow integration — ✅ DONE 2026-06-23
**Shipped:** new `dt_shadow_trades` table (`data_store._create_tables`: pair/tf/side/entry_ts/exit_ts/entry_price/exit_price/qty/pnl_net/reason, `UNIQUE(pair,entry_ts,exit_ts)`). `data_store.store_dt_shadow_trades(trades, pair, tf)` — idempotent `ON CONFLICT DO NOTHING`. Persistence wired in `main.py` DT hook (NOT in `dual_thrust_shadow.py` — tracker stays pure): on `result.new_trades`, `run_in_executor(store_dt_shadow_trades, …)` before the Telegram push. **Backfill = free:** the tracker re-replays the whole window on restart (`_last_trade_count` resets → re-emits all trades as "new"), so the first ETH-4h candle after deploy persists the full history; the upsert dedups.
**Dashboard:** `queries.get_dt_shadow` (own $10k paper book — `DT_START_BALANCE` + cumsum `pnl_net` over `exit_ts`, peak-to-trough max DD, WR/PF; guards `UndefinedTableError`→available=false). `GET /api/shadow/dt`. Models `ShadowDTResponse`/`ShadowDTTrade`/`ShadowDTPoint`. Frontend: separate "Dual Thrust shadow" card — own balance/profit/WR/PF/maxDD + own SVG equity curve (reused generalized `EquityCurve(values, start)`) + trades table. NOT merged into the ml_setups feed (separate book, tagged ETH 4h). Tests +4 (missing-table, empty, equity/WR/PF/DD math, PF-null) → 17 total pass. Mobile 375px verified.
**Live (backfill):** 7 trades, paper $10k→$8549.24, **−14.51%**, 7/7 SL (WR 0%) — the known hostile Apr–Jun ETH regime for DT. Forward real flips still accumulating (gate to Phase 1c live = ≥3–5 real forward flips; see [[project_jesse_research]]).
**Note:** bot redeployed (`docker compose up -d --build bot api web`) — required for the new table + persistence hook. Backfilled once via a one-off replay; thereafter forward-persists per 4h candle.
