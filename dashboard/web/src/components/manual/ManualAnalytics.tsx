...
function PairBreakdown({ data }: { data: Record<string, { trades: number; pnl: number; win_rate: number }> }) {
    ...
    <div className="manual-breakdown">
      {entries.map(([key, val]) => (
        <div key={key} className="manual-breakdown-row">
          ...
          <span style={{ fontSize: 11 }}>{val.trades}t</span>
          <span style={{ fontSize: 11 }}>{(val.win_rate * 100).toFixed(0)}%</span>
          <span className={`manual-breakdown-pnl ${val.pnl >= 0 ? "pnl-positive" : "pnl-negative"}`}>
            {val.pnl >= 0 ? "+" : ""}${fmt(val.pnl)}
          </span>
        </div>
      ))}
    </div>
  ...
  return (
    <div>
      ...
      <div className="manual-analytics-grid">
        <div className="manual-analytics-stat">
          ...
          <span className="manual-analytics-big">
            {(analytics.win_rate * 100).toFixed(0)}%
          </span>
          <span style={{ fontSize: 11, color: "var(--text-muted)" }}>
            {analytics.winning}W / {analytics.losing}L
          </span>
        </div>
        <div className="manual-analytics-stat">
        ...
        <div className="manual-analytics-stat">
          ...
          <span className="manual-analytics-big">
            {analytics.tp1_hit_rate != null ? (analytics.tp1_hit_rate * 100).toFixed(0) + "%" : "--"}
          </span>
        </div>
        ...
        {analytics.best_trade && (
          <div className="manual-analytics-stat">
            ...
            <span className="pnl-positive" style={{ fontSize: 13 }}>
              +${fmt(analytics.best_trade.pnl_usd)} ({analytics.best_trade.pair})
            </span>
          </div>
        )}
        {analytics.worst_trade && (
          <div className="manual-analytics-stat">
            ...
            <span className="pnl-negative" style={{ fontSize: 13 }}>
              ${fmt(analytics.worst_trade.pnl_usd)} ({analytics.worst_trade.pair})
            </span>
          </div>
        )}
      </div>
      ...
      <div style={{ marginTop: 16 }}>
        ...
        <PairBreakdown data={analytics.by_pair} />
      </div>

      <div style={{ marginTop: 12 }}>
        <div className="card-title" style={{ marginBottom: 8 }}>By Direction</div>
        <PairBreakdown data={analytics.by_direction} />
      </div>
    </div>
  );
}
