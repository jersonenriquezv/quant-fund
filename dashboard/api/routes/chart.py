...
# --- Historical retest rates (offline analysis) --------------------------
# Generated by scripts/chart_retest_stats.py; keyed "type:tf:direction".
# Loaded once per process — re-run the script + restart the API to refresh.
_RETEST_STATS_PATH = Path(__file__).resolve().parent.parent / "data" / "retest_stats.json"
_retest_stats: dict[str, dict] | None = None


def _get_retest_pct(zone_type: str, timeframe: str, direction: str) -> float | None:
    """Historical % of zones of this category that price came back to touch.

    None when the stats file is absent or the category's resolved sample is
    below the script's MIN_N floor (pct is null in the JSON in that case).
    """
    global _retest_stats
    if _retest_stats is None:
        try:
            _retest_stats = json.loads(_RETEST_STATS_PATH.read_text()).get("stats", {})
        except (OSError, json.JSONDecodeError):
            _retest_stats = {}
    entry = _retest_stats.get(f"{zone_type}:{timeframe}:{direction}")
    return entry.get("pct") if entry else None
