...
| `bybit_watcher.py` | Bybit position-watcher daemon (separate container). Polls REST every 60s, detects open/close/modify, writes `bybit_trade_annotations` + `bybit_pending_orders`, sends Telegram alerts. `_close_annotation` SUMS every `bybit_closed_pnl` row between `opened_at` and now — required for multi-partial limit closes. Journal v2: `_insert_annotation` captures `position_sl_price` + `account_equity_at_open` + `journal_schema_version=2`; `_refresh_sl` syncs SL on later attach/trail; `_backfill_daily_candles` REST-upserts 1D candles for `BYBIT_DAILY_BIAS_SYMBOLS` (feeds `context_service._htf_bias` Daily anchor). Phase 3: `_insert_annotation` + `_upsert_pending` write the `auto_*` chain (via `_V2_CHAIN_MAP`) AND pre-fill the human chain cols; on conflict `auto_*` refresh but human cols `COALESCE` (correction-safe) |
